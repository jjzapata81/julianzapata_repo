...
import { Tribe, Repository, Organization, Metric } from '../../infrastructure/db/entities/indexDB';

export const findTribeById = async (id: string): Promise<Tribe | null> => {
    return find(id);
}

export const findEnabledTribeById = async (id: string): Promise<Tribe | null> => {
    const options = {
        "state": "E",
